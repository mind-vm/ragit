...
- **xberg stays optional, so all three extractor layers exist** — this resolves §11's open question. `extract.Chain` tries xberg → `IsolatedExtractor` → `LocalExtractor`, falling back *only* on `ErrUnavailable` and never on a document-level rejection. Because the sidecar is optional, untrusted bytes really do reach local parsers, so the containment layer is load-bearing rather than a scar inherited from history.
- **A library cannot re-invoke itself the way an application can.** `valiro-go`'s `IsolatedExtractor` re-runs its own binary as a hidden CLI subcommand, which works because it *is* the application. `ragit` does not own `main()`, so the host application must call `extract.RunIsolatedChildIfInvoked()` as the first statement in `main()`. Without it the isolation layer reports itself unavailable and the chain degrades to direct local parsing — degraded, not broken, and the error message says which wiring is missing.
- **`setrlimit` is the wrong memory cap for a Go child.** `RLIMIT_AS` low enough to be useful is frequently low enough to stop a Go process from starting at all, since the runtime reserves large virtual regions up front; `RLIMIT_DATA` only covers `mmap` on Linux ≥4.7 and does nothing on macOS. The child instead sets `debug.SetMemoryLimit` (a GC target) plus a self-kill watchdog checking `runtime.MemStats.Sys` — once immediately, then every 50 ms, since parsing a small document can finish inside a single tick. This is best-effort by design: the strongest containment for a deployment is still a cgroup limit on the container, and this layer's job is to make a blow-up kill a disposable child rather than leave the choice of victim to the kernel.
- **The retention sweep needs a maintenance escape from RLS.** Finding expired rows is inherently cross-tenant — the owning tenants cannot be enumerated without a cross-tenant read. Migration 00003 adds a `ragit.maintenance` GUC to the policies' `USING` clause only, leaving `WITH CHECK` tenant-scoped, so maintenance can read and delete across tenants but can never write a row into one. This does not weaken the model: anything able to set `ragit.maintenance` could already set `ragit.tenant_id` to any value. What it buys is that the escape is visible in the schema and greppable in the code (`db.WithMaintenance`) rather than an ambient property of a connection role.
...
- **Application attributes are a separate column from extractor metadata.** `documents.metadata` is where xberg's structured output lands (§8), so an application's own key/value pairs go in `attributes` — merging them would let a new xberg field collide with an application key. They are filterable by JSONB containment, GIN indexed, and denormalized onto chunks for the same reason the scope columns are: retrieval filters on them alongside the vector scan, and doing that through a join fights the HNSW index rather than riding it. Same obligation too — `SetDocumentAttributes` re-stamps the chunks, because the copy does not self-heal and reprocessing will not fix it. Crucially they **narrow rather than confine**: an empty filter matches everything, the opposite of `Scope`'s rule, so they must not be used for access control.
...
- How wide the local fallback's format set should be. It currently covers text/Markdown, CSV/TSV, JSON, HTML/XML, PDF (text layer only, no OCR) and DOCX. Spreadsheets are deliberately absent: `excelize` allocates whatever dimension range a file declares, and adding it means taking on exactly the allocation profile §6 is about. Worth adding only if a real deployment turns out to run without a sidecar *and* needs xlsx.
